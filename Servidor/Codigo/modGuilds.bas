...
Public Function m_EsGuildSubLeader(ByRef PJ As String, ByVal GuildIndex As Integer) As Boolean
    m_EsGuildSubLeader = (UCase$(PJ) = UCase$(Trim$(Guilds(GuildIndex).GetSubLider)))
End Function
...
Dim GI          As Integer
    ...
    If UserIndex > 0 Then
        ...
        If GI > 0 Then
            ...
            If m_EsGuildSubLeader(Expulsado, GI) Then
             Call WriteVar(GUILDINFOFILE, "GUILD" & GI, "SubLider", "")
            End If
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).Stats.ELV < 25 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        refError = "Para fundar un clan debes ser nivel 25 y tener 100 en liderazgo."
        Exit Function
    End If
    
    If TieneObjetos(939, 1, UserIndex) = False Then
            refError = "Necesitas el Talisman de Lider para fundar un clan."
            Exit Function
    End If
    
    If TieneObjetos(1048, 1, UserIndex) = False Then
            refError = "Necesitas el Acta de Fundacion para fundar un clan."
            Exit Function
    End If
...
Dim i       As Integer
    ...
    tStr = tStr & Guilds(GI).GetSubLider & "¬"
...
Dim NroAspirante    As Integer
    ...
    If Not m_EsGuildLeader(UserList(UserIndex).name, GI) Or Not m_EsGuildSubLeader(UserList(UserIndex).name, GI) Then
        Exit Function
    End If
...
Dim GuildActual As Integer
    ...
    If Not m_EsGuildLeader(UserList(UserIndex).name, GI) And Not m_EsGuildSubLeader(UserList(UserIndex).name, GI) Then
