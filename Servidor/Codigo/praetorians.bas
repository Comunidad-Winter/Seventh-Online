...
On Error GoTo errorh
    ...
    Else
        If Not (Npclist(npcind).Invent.BarcoSlot = 6) Then
            ...
            Call SendData(SendTarget.ToMap, npcind, Npclist(npcind).pos.Map, "XFC" & Npclist(npcind).Char.CharIndex & "," & 0 & "," & 0)
        End If
...
'        azar = 1
    ...
    Case 3
        ...
        Call SendData(SendTarget.ToMap, npcind, Npclist(npcind).pos.Map, "XFC" & Npclist(npcind).Char.CharIndex & "," & FXIDs.FXMEDITARGRANDE & "," & LoopAdEternum)
        ''UserList(UserIndex).Char.FX = FXIDs.FXMEDITARGRANDE
    
        If Npclist(npcind).CanAttack = 1 Then
            Npclist(npcind).CanAttack = 0
            bs = Npclist(npcind).Invent.MonturaSlot
            ...
            Else
                Npclist(npcind).Invent.MonturaSlot = bs
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, NPCAlInd, Npclist(NPCAlInd).pos.Map, "XFC" & Npclist(NPCAlInd).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, Paralizado, Npclist(Paralizado).pos.Map, "XFC" & Npclist(Paralizado).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
    ...
    'Call SendData(SendTarget.ToNPCArea, Paralizado, Npclist(Paralizado).Pos.Map, "XFC" & Npclist(Paralizado).Char.CharIndex & "," & Hechizos(Npclist(paralizador).Spells(1)).FXgrh & "," & Hechizos(Npclist(paralizador).Spells(1)).loops)
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, curado, Npclist(curado).pos.Map, "XFC" & Npclist(curado).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
    ...
    'Call SendData(SendTarget.ToNPCArea, curado, Npclist(curado).Pos.Map, "XFC" & Npclist(curado).Char.CharIndex & "," & Hechizos(Npclist(curador).Spells(3)).FXgrh & "," & Hechizos(Npclist(curador).Spells(3)).loops)
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).pos.Map, "||" & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(npcind).Char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).pos.Map, "XFC" & UserList(PJEnInd).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).pos.Map, "||" & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(npcind).Char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).pos.Map, "XFC" & UserList(PJEnInd).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops + 4)
    ...
    'Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).Pos.Map, "XFC" & UserList(PJEnInd).Char.CharIndex & "," & Hechizos(4).FXgrh & "," & Hechizos(4).loops + 4)
            ...
            For X = 8 To 95
                For Y = 8 To 95
                    ...
                    If PJInd > 0 Then
                        ...
                        Call SendData(SendTarget.ToIndex, PJInd, 0, "||" & Npclist(npcind).name & " te ha quitado " & danioI & " puntos de vida al romper su vara." & FONTTYPE_BORDON)
                        Call SendData(SendTarget.ToPCArea, PJInd, UserList(PJInd).pos.Map, "TW" & Hechizos(indireccion).WAV)
                        Call SendData(SendTarget.ToPCArea, PJInd, UserList(PJInd).pos.Map, "XFC" & UserList(PJInd).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
                    ...
                    ElseIf (MascotaInd > 0) Then
                        If (Npclist(MascotaInd).MaestroUser > 0) Then
                            ...
                            Call SendData(SendTarget.ToNPCArea, MascotaInd, Npclist(MascotaInd).pos.Map, "XFC" & Npclist(MascotaInd).Char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
