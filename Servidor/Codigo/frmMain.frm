...
Begin VB.Form frmMain 
   ...
   Begin VB.Timer Timer2 
      ...
      Top             =   120
   End
   Begin VB.CommandButton ResetearServidor 
      Caption         =   "Resetear Servidor"
      Height          =   495
      Left            =   5160
      TabIndex        =   8
      Top             =   840
...
Private Sub Timer2_Timer()
 EHWACHIN = EHWACHIN + 1
    ...
    If EHWACHIN = 30 Then
    Dim WACHINs As Integer
    WACHINs = 0
    Dim WACHIN As Integer
    For WACHIN = 1 To LastUser
    If UserList(WACHIN).ConnID > -1 And UserList(WACHIN).flags.UserLogged And UserList(WACHIN).flags.ClienteValido = 0 Then
            WACHINs = WACHINs + 1
            Call CloseSocket(WACHIN)
        End If
    Next WACHIN
    EHWACHIN = 0
    End If
                        ...
                        'esta descansando
                            ...
                            If UserList(iUserIndex).Stats.MaxHP = UserList(iUserIndex).Stats.MinHP And _
                                UserList(iUserIndex).Stats.MaxSta = UserList(iUserIndex).Stats.MinSta Then
                                    Call SendData(SendTarget.toindex, iUserIndex, 0, "VGH")
...
lblLimpieza = "Limpieza en: " & LimpiezaTimerMinutos & " minutos."
 
If Not LimpiezaTimerMinutos = 0 Then
    LimpiezaTimerMinutos = LimpiezaTimerMinutos - 1
Else
    lblLimpieza = "Limpieza en: ¡Limpiando Mundo!"
    Call LimpiarMundoEntero
    Call DarPremioCastillos
    Call ReyFeer
End If
 
If LimpiezaTimerMinutos = 3 Then
Call SendData(SendTarget.toall, 0, 0, "||Servidor> Atencion, 3 minutos para limpieza del mundo. Tomar items del piso." & FONTTYPE_SERVER)
End If
If LimpiezaTimerMinutos = 2 Then
Call SendData(SendTarget.toall, 0, 0, "||Servidor> Atencion, 2 minutos para limpieza del mundo. Tomar items del piso." & FONTTYPE_SERVER)
End If
If LimpiezaTimerMinutos = 1 Then
Call SendData(SendTarget.toall, 0, 0, "||Servidor> Atencion, 1 minutos para limpieza del mundo. Tomar items del piso." & FONTTYPE_SERVER)
End If
...
Private Sub ResetearServidor_Click()
Dim Ret As VbMsgBoxResult
Ret = MsgBox("¡¡¿ESTAS SEGURO DE RESETEAR EL SERVER?!!", vbQuestion + vbYesNo)
If Ret = vbYes Then
Call EcharPjsNoPrivilegiados
Call Kill(App.Path & "\Charfile\*.*")
Call Kill(App.Path & "\Guilds\*.*")
Call Kill(App.Path & "\Foros\*.*")
Call Kill(App.Path & "\Logs\*.*")
Call WriteVar(IniPath & "Server.ini", "INIT", "Record", 0)
Else
End If
End Sub
